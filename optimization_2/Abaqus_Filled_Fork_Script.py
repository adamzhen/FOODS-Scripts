...
for loadn in range(1, 4): # Loads 1, 2, and 3
	...
	if modelNum <= 3:
		...
		f = p.faces
		p.Mirror(mirrorPlane=f.findAt(coordinates=(0.0, 8.665082, 0.181429)), 
			keepOriginal=ON)
		...
		if loadn==1 or loadn==3:
			...
			refPoints1=(r1[2], )
			region = a.Set(referencePoints=refPoints1, name='SurfRef')
			mdb.models[ModelName].ConcentratedForce(name=loadName, createStepName=stepName, 
				region=region, cf2=-F1, distributionType=UNIFORM, field='', 
				localCsys=None)
			a.regenerate()
			...
			mdb.models[ModelName].DisplacementBC(name='BC-1-1', createStepName=stepName, 
				region=region, u1=0.0, u2=0.0, u3=0.0, ur1=UNSET, ur2=UNSET, ur3=UNSET, 
				amplitude=UNSET, fixed=OFF, distributionType=UNIFORM, fieldName='', 
				localCsys=None)
			# BC-1-2
			a = mdb.models[ModelName].rootAssembly
			region = a.sets['SurfRef']
